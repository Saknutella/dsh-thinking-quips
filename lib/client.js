...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		// ── morph geometry: end ──────────────────────────────────────────────
		const DEFAULTS = { enabled: true, color: SHIMMER, colorTheme: false, langMode: "ui", quips: DEFAULT_QUIPS, perLang: {}, quipMs: 8000, glow: 35, indicatorOnly: false, loader: "orbit", loaderSize: "md", textEffect: "shimmer", speed: "normal" };
		/**
		...
		*/
		...
		/** DSH's own shimmer duration; scaling it means restating it. */
		...
		*/
		...
		/**
		* Apply the speed. Everything the plugin animates reads `var(--tq-speed)` in the
		* plugin stylesheet; DSH's shimmer is not ours, so at any level other than normal a
		* small override restates its duration scaled — and at "normal" both the variable
		* and the override are removed, leaving the authentic rule untouched.
		* @param cfg - the config (reads `speed`).
		*/
