...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		* Radial outline of one rounded polygon, sampled at {@link MORPH_SAMPLES}
		* angles — sample 0 is 12 o'clock, same convention as {@link morphPath}, so a
		* shape's first corner lands exactly on it.
		*
		* Each radius is the polar form of a regular polygon (`inradius / cos x`, x =
		* the angle from the nearest edge normal) with the corners cut at
		* {@link MORPH_CHAMFER} of the half-sector, then normalised so the outermost
		* point is exactly 1. A polygon rather than Gielis' superformula (tried first,
		* discarded): the superformula's corner-to-edge contrast saturates near 1.41,
		* i.e. only a ~25% radius swing, so at 16px a "triangle" read as a wobbly
		* circle. The cut corners plus `stroke-linejoin:round` still look rounded.
		...
		*/
		function morphProfile(kind) {
			...
			for (let i = 0; i < MORPH_SAMPLES; i++) {
				if (shape.corners < 3) { radii.push(1); continue; }
				const a = (i / MORPH_SAMPLES) * Math.PI * 2 - Math.PI / 2;
				...
				radii.push(inradius / Math.cos(Math.min(x, cut)));
			}
			let max = 0;
		...
		/**
		* Easing for the turn. A quintic ease-in-out instead of the morph's cubic: it
		* holds longer at each end, so the rotation visibly lags the shape, drifts to
		* the next orientation and settles — without overshooting, which would read as
		* a stutter because the shape keeps turning one way for the whole cycle.
		*/
		...
		/**
		* The outline at one point of the cycle.
		...
		*/
		function morphPath(segment, progress) {
			...
			for (let i = 0; i < MORPH_SAMPLES; i++) {
				...
				d += (i === 0 ? "M" : "L") + (half + Math.cos(a) * r).toFixed(2) + " " + (half + Math.sin(a) * r).toFixed(2);
			}
			return d + "Z";
		}
		...
		*/
		function ensureMorph(span) {
			...
			const frame = () => {
				...
				const elapsed = (Date.now() - started) % MORPH_CYCLE_MS;
				const segment = Math.floor(elapsed / segmentMs);
				const progress = (elapsed % segmentMs) / segmentMs;
				try {
					path.setAttribute("d", morphPath(segment, progress));
					svg.setAttribute("transform", "rotate(" + morphRotation(segment, progress).toFixed(2) + " 8 8)");
