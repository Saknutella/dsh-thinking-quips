...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULTS = { enabled: true, color: SHIMMER, langMode: "ui", quips: DEFAULT_QUIPS, perLang: {}, quipMs: 8000, glow: 35, indicatorOnly: false };
		...
		*/
		...
		const CSS = [
			...
			"@media (prefers-reduced-motion:reduce){.tq-orbit i{animation:none;opacity:.55}}"
		].join("");
		...
		const zh = {
			...
			"quips.indicatorOnlyDesc": "开启后只注入点阵图标，不替换状态文字——可与其他修改状态文字的插件共存。"
		};
		const en = {
			...
			"quips.indicatorOnlyDesc": "Inject only the dot-orbit icon and leave the status text alone, so another status-text plugin can coexist."
		};
				...
				if (typeof MutationObserver !== "undefined" && document.body) {
					observer = new MutationObserver(() => {
						try {
							...
							ensureOrbit(el);
							const cfg = store().get();
							if (cfg.indicatorOnly === true) return; // indicator only
							const quips = selectPhrases(cfg, activeLocale);
