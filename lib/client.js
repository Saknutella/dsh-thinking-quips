	...
	factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			"quips.close": "关闭",
			"quips.add": "添加一条",
			"quips.placeholder": "输入一条俏皮话，例如：“正在搬砖…”",
			"quips.empty": "暂无自定义俏皮话，将使用语言默认列表。",
			...
			"quips.glow": "发光强度",
			"quips.mixin": "混合显示",
			"quips.mixinDesc": "开启后，中英文可混合；关闭时按界面语言显示（非中文界面显示英语）。",
		...
		const en = {
			...
			"quips.close": "Close",
			"quips.add": "Add one",
			"quips.placeholder": "Type a quip, e.g. “Sifting through context…”",
			"quips.empty": "No custom quips yet; the language default list is used.",
			...
			"quips.mode.both": "English + Chinese",
			...
			"quips.mixin": "Mixed display",
			"quips.mixinDesc": "On: allow EN and CN to mix. Off: follow the UI language (non-Chinese UI shows English).",
