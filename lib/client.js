...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		// ── morph geometry: end ──────────────────────────────────────────────
		const DEFAULTS = { enabled: true, color: SHIMMER, colorTheme: false, langMode: "ui", quips: DEFAULT_QUIPS, perLang: {}, quipMs: 8000, glow: 35, indicatorOnly: false, loader: "orbit", loaderSize: "md", textEffect: "shimmer", speed: 1 };
		/**
		* One global speed multiplier for every animation the plugin drives — the loading
		* icons, the shape morph and the text wave. 1 is the original pace; 2 is twice as
		* fast. Everything is CSS `calc(<base> / var(...))` (or, for the morph's frame
		* loop, elapsed time multiplied), so there is exactly one number to reason about.
		* DSH's own shimmer sweep is not scaled — see {@link applySpeedCSS}.
		*/
		...
		*/
		...
		// ── text effects: the shimmer, or a wave through the line ────────────
		/** Selectable status-text effects. `shimmer` is DSH's own sweep, untouched. */
		...
		*/
		...
		// ── wave tokens: end ─────────────────────────────────────────────────
		/** The wave container inside a status element, if it is there. */
		function waveSpan(el) {
			return el && typeof el.querySelector === "function" ? el.querySelector(".tq-wave") : null;
		}
		/** Remove the wave container (switching back to the shimmer). */
		function clearWave(el) {
			const span = waveSpan(el);
			if (span !== null && typeof span.remove === "function") span.remove();
			try {
		...
		*
		* The label node DSH/React owns is KEPT and merely blanked: deleting a node React
		* still holds is how a plugin breaks the shell, and blanking it also means the
		* text stays updating through React (we re-read and re-blank on the next pass).
		...
		*/
		function renderWave(el, text) {
			const node = quipNode(el);
			if (node !== null) {
				// Remember DSH's own label before hiding it, so the line can be handed back
				// when the quip list turns out to be empty. First save wins (that is the label
				// DSH rendered before we took the line over), but a fresh DSH label — after a
				// locale switch, say — refreshes it.
				if (node.nodeValue !== "" && typeof el.setAttribute === "function") {
					const looksLikeDsh = /deep diving|深度求索/i.test(node.nodeValue);
					if (looksLikeDsh || el.getAttribute("data-tq-label") === null) el.setAttribute("data-tq-label", node.nodeValue);
				}
		...
		/**
		* Give the line back to DSH: drop the wave and restore the label we hid. Called
		* when there is no quip to show (an empty section), which in wave mode would
		* otherwise leave the status line blank — the shimmer path never touched the label,
		* so this is a no-op there.
		* @param el - the running-status element.
		*/
		function restoreStatusText(el) {
			clearWave(el);
			const node = quipNode(el);
			if (node === null || node.nodeValue !== "") return;
			const saved = typeof el.getAttribute === "function" ? el.getAttribute("data-tq-label") : null;
			if (saved) node.nodeValue = saved;
		}
		/**
		* Put `text` on the status line with whichever effect is configured. The single
		* entry point, so switching effects can never leave both renderings on screen.
		* @param el - the running-status element.
		* @param text - the quip to show.
		* @param cfg - the config (reads `textEffect`).
		*/
		function applyStatusText(el, text, cfg) {
			const effect = cfg && TEXT_EFFECTS.indexOf(cfg.textEffect) !== -1 ? cfg.textEffect : "shimmer";
			if (effect === "wave") { renderWave(el, text); return; }
			clearWave(el);
			const node = quipNode(el);
			if (node !== null && node.nodeValue !== text) node.nodeValue = text;
		}
		/**
		...
		*/
		...
		/**
		* Apply the global speed multiplier. Every animation the PLUGIN owns reads
		* `var(--tq-speed)` in the plugin stylesheet (the loaders, the shape morph's frame
		* loop, the wave), and at exactly 1 the variable is removed so the default build
		* leaves no trace at all.
		*
		* DSH's own shimmer sweep is deliberately NOT scaled: it keeps the pace DSH ships,
		* whatever this setting says. Overriding another package's animation duration is
		* the kind of thing that ages badly, and the shimmer is the baseline the other
		* effects are compared against.
		...
		*/
		...
		/** The configured speed, clamped (1 when unset or invalid). */
		function speedOf(cfg) {
			const raw = Number(cfg && cfg.speed);
			return Number.isFinite(raw) && raw > 0 ? clamp(raw, SPEED_MIN, SPEED_MAX) : 1;
		}
		...
		const CSS = [
			...
			"@media (prefers-reduced-motion:reduce){.tq-loader i,.tq-loader svg,.tq-waveItem{animation:none!important}.tq-loader-orbit i,.tq-loader-dots i,.tq-loader-pulse i,.tq-loader-bars i{opacity:.7}.tq-loader-bars i{transform:scaleY(.7)}.tq-loader-orbit i.blank{opacity:0}}"
		].join("");
		...
		const zh = {
			...
			"quips.glow": "发光强度",
			"quips.speed": "动画速度",
			"quips.speedUnit": "×",			"quips.textEffect": "文字特效",
			"quips.textEffectDesc": "状态行文字的动效：扫光是 DSH 原版那道横向流光，波浪让每个字（中文）或每个词（英文）依次向上跳一下。",
		...
		const en = {
			...
			"quips.textEffectDesc": "What the status text does: Shimmer is DSH's own sideways sweep, Wave lifts each character (CJK) or word (Latin) in turn.",
		...
		function QuipsSettingsRow({ t }) {
			...
			const speedValue = Math.round(speedOf(cfg) * 10) / 10;
			...
			return jsxs("div", { className: "tq-row", children: [
				...
				jsxs("div", { className: "tq-timeRow", children: [
					jsx("span", { className: "tq-timeLabel", children: t("quips.speed") }),
					jsx("input", { type: "number", className: "tq-timeInput", min: SPEED_MIN, max: SPEED_MAX, step: 0.1, value: speedValue, onChange: (e) => { const s = clamp(Math.round((Number(e.currentTarget.value) || 1) * 10) / 10, SPEED_MIN, SPEED_MAX); patchConfig({ speed: s }); } }),
					jsx("span", { className: "tq-timeUnit", children: t("quips.speedUnit") })
				] }),
		...
		function apply(ctx) {
			...
			const timer = setInterval(() => {
				try {
					...
					if (!quips.length) { restoreStatusText(el); return; }
					const idx = Math.floor((Date.now() - start) / (cfg.quipMs || 8000)) % quips.length;
					applyStatusText(el, quips[idx], cfg);
			...
			try {
				...
				if (el0) {
					...
					if (cfg0.indicatorOnly !== true) {
						...
						if (quips0.length) applyStatusText(el0, quips0[0], cfg0);
						else restoreStatusText(el0);
					}
				}
			} catch (_) { /* noop */ }
				...
				if (typeof MutationObserver !== "undefined" && document.body) {
					observer = new MutationObserver(() => {
						try {
							...
							const quips = selectPhrases(cfg, activeLocale);
							if (quips.length) applyStatusText(el, quips[0], cfg);
						} catch (_) { /* noop */ }
					});
