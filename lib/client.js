	...
	factory: (require) => {
		...
		*/
		...
		const CSS = [
			...
			// ring: a real ring — a faint full track under a bright half-arc that sweeps
			// around it. Two coloured borders alone read as a "C" in a still frame; the
			// track is what makes it a ring at every animation phase.
			".tq-loader-ring{display:block;width:16px;height:16px;position:relative}",
			".tq-loader-ring::before{content:'';position:absolute;inset:0;box-sizing:border-box;border-radius:50%;border:2px solid currentColor;opacity:.2}",
			".tq-loader-ring i{position:absolute;inset:0;box-sizing:border-box;border-radius:50%;border:2px solid transparent;border-top-color:currentColor;border-right-color:currentColor;animation:tq-spin .8s linear infinite}",
