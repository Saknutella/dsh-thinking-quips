...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* icons, the shape morph, the text wave, and DSH's own shimmer sweep. 1 is the
		* original pace; 2 is twice as fast. Everything is CSS `calc(<base> / var(...))`
		* (or, for the morph's frame loop, elapsed time multiplied), so there is exactly
		* one number to reason about.
		*/
		const SPEED_VAR = "--tq-speed";
		const SPEED_MIN = 0.25;
		const SPEED_MAX = 3;
		/** DSH's own shimmer duration; the speed override has to restate it. */
		...
		*/
		...
		/**
		* Apply the global speed multiplier. Our own animations read `var(--tq-speed)` in
		* the plugin stylesheet; DSH's shimmer is not ours, so at any pace other than 1 a
		* small override restates its duration scaled — and at exactly 1 the override is
		* removed entirely, leaving the authentic rule untouched.
		* @param cfg - the config (reads `speed`).
		*/
		function applySpeedCSS(cfg) {
			if (typeof document === "undefined") return;
			const raw = Number(cfg && cfg.speed);
			const speed = Number.isFinite(raw) && raw > 0 ? clamp(raw, SPEED_MIN, SPEED_MAX) : 1;
			...
			const css = `[role="status"][class*="turnStatus"]{animation-duration:calc(${SHIMMER_MS}ms / var(${SPEED_VAR},1)) !important}`;
			if (!el) {
				el = document.createElement("style");
