...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*
		* The blended radii are turned into points and then re-centred on their own
		* bounding box. Blending two box-centred profiles does not stay exactly
		* box-centred (a circle and a triangle do not agree about where the middle is),
		* and the rotation pivot is the box centre — so without this last nudge the
		* silhouette would still creep up and down across a segment.
		...
		*/
		function morphPath(segment, progress) {
			...
			const box = morphBounds(points);
			const half = RING_BOX / 2;
			let d = "";
			for (let i = 0; i < points.length; i++) {
				const x = half + points[i][0] - box.cx;
				const y = half + points[i][1] - box.cy;
		...
		*/
		function morphFrame(elapsedMs) {
			...
			const half = RING_BOX / 2;
			return {
				d: morphPath(segment, progress),
				rotation: "rotate(" + morphRotation(segment, progress).toFixed(2) + " " + half + " " + half + ")",
				segment,
				progress
			};
		}
		...
		function ensureMorph(span) {
			...
			const frame = () => {
				...
				try {
					...
					// different epoch and would produce a nonsense elapsed value.
					const step = morphFrame(Date.now() - started);
					path.setAttribute("d", step.d);
					svg.setAttribute("transform", step.rotation);
